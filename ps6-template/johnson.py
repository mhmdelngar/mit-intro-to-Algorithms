...
def johnson(n, S):
    ...
    for m in range(n) :
        w[s][m]=0
    args=bellman_ford(Adj,lambda u,v: w[u][v],s)
    if args is None:
        return None
    d,_=args
    for adi in range(n+1):
        for adjWeight in w[adi]:
            if w[adi][adjWeight] <INF:
                w[adi][adjWeight]=w[adi][adjWeight]+d[adi]-d[adjWeight]
    Adj.pop()
    D = [[INF for _ in range(n)] for _ in range(n)]
    for adji in range(n):
        dOfDikstra,_=dijkstra(Adj,lambda u,v: w[u][v],adji)
        for adjican in range(n):
            if dOfDikstra[adjican]-d[adji]+d[adjican]> INF or dOfDikstra[adjican] == INF:
                pass
            else :
                dOfDikstra[adjican]=dOfDikstra[adjican]-d[adji]+d[adjican]
